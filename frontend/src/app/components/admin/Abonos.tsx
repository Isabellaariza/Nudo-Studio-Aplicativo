...
import { DollarSign, Search, Plus, Info, X, CheckCircle, XCircle, Receipt, Ban, AlertTriangle, Download, History, UploadCloud } from 'lucide-react';
...
interface Abono {
  ...
  fechaAbono: string;
  estado: 'por_verificar' | 'aprobado' | 'completo' | 'rechazado' | 'cancelado' | 'exceso';
  id_estudiante?: number;
  id_taller?: number;
  id_matricula?: number;
  comprobante_pago?: string | null;
  motivo_rechazo?: string | null;
  comprobantes?: { id_comprobante: number; url: string; subido_por: string; fecha_subida: string }[];
  historial?: { tipo: string; nota: string; comprobante_devolucion: string | null; monto_exceso: number | null; creado_en: string }[];
}
...
const motivoFinal = motivoSeleccionado === 'Otro motivo (especificar abajo)' ? motivoCustom : motivoSeleccionado;
  ...
  const abrirModalAccion = (id: number, accion: 'rechazar' | 'exceso') => {
    setRechazarId(id);
  ...
  const handleRechazar = async () => {
    ...
    if (modalAccion === 'rechazar') {
      ...
      } catch (err: any) { toast.error(err.message || 'Error al rechazar'); return; }
    } else {
      try {
        await abonosAPI.marcarExceso(rechazarId, {
                        ...
                        <div style={{ display: 'flex', gap: '4px', justifyContent: 'center', alignItems: 'center' }}>
                          ...
                          {a.estado === 'por_verificar' && (
                            <>
                              ...
                              <Tooltip text="Rechazar / Exceso de pago">
                                <motion.button whileHover={{ scale: 1.15 }} onClick={() => abrirModalAccion(a.id, 'rechazar')}
                                  style={{ padding: '7px', borderRadius: '8px', border: 'none', background: 'transparent', cursor: 'pointer' }}>
      ...
      {showInfoModal && selected && (
        <Modal isOpen={true} onClose={() => setShowInfoModal(false)} title="Detalles del Abono">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            {selected.comprobantes && selected.comprobantes.length > 0 && (
              <div>
                <div style={{ fontSize: '12px', fontWeight: 700, color: '#2D4B39', marginBottom: '10px', letterSpacing: '0.05em' }}>
                  COMPROBANTES DE PAGO ({selected.comprobantes.length})
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {selected.comprobantes.map((c, idx) => (
                    <div key={c.id_comprobante} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', background: c.subido_por === 'admin' ? 'rgba(59,130,246,0.05)' : 'rgba(45,75,57,0.04)', border: `1px solid ${c.subido_por === 'admin' ? 'rgba(59,130,246,0.15)' : 'rgba(45,75,57,0.08)'}` }}>
                      <span style={{ fontSize: '11px', fontWeight: 700, color: '#9CA3AF', minWidth: '20px' }}>#{idx + 1}</span>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: '12px', fontWeight: 600, color: c.subido_por === 'admin' ? '#1D4ED8' : '#2D4B39' }}>
                          {c.subido_por === 'admin' ? '🏢 Subido por admin (devolución)' : '👤 Subido por cliente'}
                        </div>
                        <div style={{ fontSize: '11px', color: '#9CA3AF' }}>
                          {new Date(c.fecha_subida).toLocaleString('es-CO', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                        </div>
                      </div>
                      <motion.button whileHover={{ scale: 1.05 }} onClick={() => setShowComprobante(c.url)}
                        style={{ padding: '6px 12px', borderRadius: '8px', border: '1px solid rgba(45,75,57,0.15)', background: '#fff', fontSize: '11px', fontWeight: 600, color: '#2D4B39', cursor: 'pointer' }}>
                        Ver
                      </motion.button>
                    </div>
                  ))}
            ...
            {selected.historial && selected.historial.length > 0 && (
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
                  ...
                  <span style={{ fontSize: '12px', fontWeight: 700, color: '#2D4B39', letterSpacing: '0.05em' }}>HISTORIAL</span>
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                  {selected.historial.map((h, idx) => (
                    <div key={idx} style={{ padding: '10px 12px', borderRadius: '10px', background: h.tipo === 'exceso_detectado' ? 'rgba(245,158,11,0.06)' : h.tipo === 'devolucion_registrada' ? 'rgba(59,130,246,0.05)' : 'rgba(45,75,57,0.03)', border: `1px solid ${h.tipo === 'exceso_detectado' ? 'rgba(245,158,11,0.2)' : h.tipo === 'devolucion_registrada' ? 'rgba(59,130,246,0.12)' : 'rgba(45,75,57,0.07)'}` }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: '8px' }}>
                        <div>
                          <span style={{ fontSize: '11px', fontWeight: 700, color: h.tipo === 'exceso_detectado' ? '#B45309' : h.tipo === 'devolucion_registrada' ? '#1D4ED8' : '#2D4B39' }}>
                            {h.tipo === 'exceso_detectado' ? '⚠️ Exceso detectado' : h.tipo === 'devolucion_registrada' ? '↩️ Devolución registrada' : h.tipo === 'reenvio_comprobante' ? '📎 Comprobante reenviado' : h.tipo === 'creacion' ? '✅ Abono creado' : h.tipo}
                          </span>
                          {h.nota && <div style={{ fontSize: '12px', color: '#4B5563', marginTop: '3px' }}>{h.nota}</div>}
                          {h.monto_exceso && <div style={{ fontSize: '12px', fontWeight: 700, color: '#B45309', marginTop: '2px' }}>Exceso: ${Number(h.monto_exceso).toLocaleString('es-CO')} COP</div>}
                        </div>
                        <span style={{ fontSize: '10px', color: '#9CA3AF', whiteSpace: 'nowrap', flexShrink: 0 }}>
                          {new Date(h.creado_en).toLocaleString('es-CO', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}
                        </span>
                      </div>
                    </div>
                  ))}
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                  ...
                  <div>
                    ...
                    <p style={{ fontSize: '12px', color: '#6B7280', margin: '2px 0 0' }}>Se notificará al cliente por correo</p>
                  </div>
                </div>
                  ...
                  <motion.div key="exceso" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                    <div style={{ padding: '14px 16px', borderRadius: '12px', background: 'rgba(245,158,11,0.06)', border: '1px solid rgba(245,158,11,0.2)', marginBottom: '16px' }}>
                      <p style={{ fontSize: '13px', color: '#92400E', margin: 0, lineHeight: 1.5 }}>
                        El cliente pagó <strong>más de lo que debía</strong>. Se le enviará un correo informándole que su inscripción está confirmada y que nos comunicaremos para devolver el excedente.
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <label style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 14px', borderRadius: '10px', border: `1.5px dashed ${compDevolucion ? '#10B981' : 'rgba(245,158,11,0.3)'}`, background: compDevolucion ? 'rgba(16,185,129,0.04)' : '#fff', cursor: 'pointer', fontSize: '13px', color: compDevolucion ? '#059669' : '#6B7280' }}>
                        ...
                        <input type="file" accept="image/*,.pdf" className="hidden"
                          onChange={e => { const f = e.target.files?.[0]; if (f) handleUploadCompDevolucion(f); }}
                          disabled={uploadingComp} style={{ display: 'none' }} />
                      </label>
                      <p style={{ fontSize: '11px', color: '#9CA3AF', marginTop: '4px' }}>También puedes subirlo después desde el detalle del abono.</p>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
