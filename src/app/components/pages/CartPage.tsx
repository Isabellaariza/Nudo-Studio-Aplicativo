...
import { ShoppingBag, Plus, Minus, Trash2, Upload, CheckCircle, RefreshCw, AlertCircle, X, Tag } from 'lucide-react';
import { toast } from 'sonner';
import { pedidosAPI, descuentosAPI } from '../../lib/api';
...
export function CartPage({ user, onNavigate, onCartUpdate }: CartPageProps) {
  ...
  const [descuentosActivos, setDescuentosActivos] = useState<any[]>([]);
  ...
  // Limpiar carrito si el usuario cierra sesión o no está autenticado
  useEffect(() => {
    if (!user) {
      setCartItems([]);
  ...
  useEffect(() => {
    descuentosAPI.getActivos().then(d => setDescuentosActivos(d.descuentos || [])).catch(() => {});
  }, []);
  ...
  // ── LÓGICA DE DESCUENTOS POR PRODUCTO ──────────────────────────────────────
  // Obtiene el mejor descuento (mayor porcentaje) aplicable a un ítem según su cantidad
  const getDescuentoItem = (item: CartItem) => {
    const reglas = descuentosActivos
      .filter(d => String(d.id_producto) === String(item.id) && item.quantity >= Number(d.cantidad_minima))
      .sort((a, b) => Number(b.porcentaje) - Number(a.porcentaje));
    return reglas[0] || null;
  };

  // Obtiene la próxima promoción disponible para un ítem (para mostrar al usuario)
  const getProximoDescuento = (item: CartItem) => {
    const reglas = descuentosActivos
      .filter(d => String(d.id_producto) === String(item.id) && item.quantity < Number(d.cantidad_minima))
  ...
  // Precio efectivo de un ítem (con descuento si aplica)
  const getPrecioEfectivo = (item: CartItem) => {
    const desc = getDescuentoItem(item);
    if (!desc) return item.price;
    return item.price - (item.price * Number(desc.porcentaje)) / 100;
  };

  const subtotal = cartItems.reduce((sum, item) => sum + getPrecioEfectivo(item) * item.quantity, 0);
  const descuentoTotal = cartItems.reduce((sum, item) => {
    const desc = getDescuentoItem(item);
    if (!desc) return sum;
    return sum + (item.price - getPrecioEfectivo(item)) * item.quantity;
  }, 0);
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      // 3. Mapear los productos del carrito (con precio efectivo que incluye descuento)
      const productosMapeados = cartItems.map(item => ({
        id_producto: Number(item.id),
        cantidad: item.quantity,
        precio_unitario: getPrecioEfectivo(item),
      }));
            ...
            >
              ...
              >
                ...
                <p className="text-sm mb-5" style={{ color: 'rgba(45,75,57,0.6)', lineHeight: 1.6 }}>
                  Tu pedido fue registrado exitosamente. Si hay alguna novedad con tu pedido, se te enviará un <strong style={{ color: '#2D4B39' }}>email</strong> o mensaje vía <strong style={{ color: '#2D4B39' }}>WhatsApp</strong> para que el asesor pueda comunicarse contigo.
                </p>
              ...
              {cartItems.map((item, index) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(() => {
                      const desc = getDescuentoItem(item);
                      const siguiente = getProximoDescuento(item);
                      ...
                      return (
                        <>
                          <div className="flex items-center gap-2 mb-1">
                            {desc ? (
                              <>
                                <p className="font-elegant text-lg" style={{ color: '#10B981' }}>
                                  ${precioEfectivo.toLocaleString('es-CO')} COP
                                </p>
                                <span style={{ textDecoration: 'line-through', fontSize: '12px', color: '#9CA3AF' }}>
                                  ${item.price.toLocaleString('es-CO')}
                                </span>
                                <span className="px-1.5 py-0.5 rounded-full text-xs font-bold" style={{ background: 'rgba(16,185,129,0.1)', color: '#10B981' }}>
                                  -{desc.porcentaje}%
                                </span>
                              </>
                            ) : (
                              <p className="font-elegant text-lg mb-0" style={{ color: '#B8860B' }}>
                          ...
                          {desc && (
                            <div className="flex items-center gap-1 mb-2">
                              ...
                              <span className="text-xs" style={{ color: '#10B981' }}>Descuento del {desc.porcentaje}% aplicado</span>
                            </div>
                          )}
                          {/* Aviso de próxima promo */}
                          {!desc && siguiente && (
                            <div className="flex items-center gap-1 mb-2">
                              <Tag className="w-3 h-3" style={{ color: '#B8860B' }} />
                              <span className="text-xs" style={{ color: '#B8860B' }}>
                                Agrega {Number(siguiente.cantidad_minima) - item.quantity} más y obtén {siguiente.porcentaje}% OFF
                              </span>
                            </div>
                          )}
                        </>
                      ...
                      <div className="flex items-center gap-1 rounded-full p-1" style={{ backgroundColor: 'rgba(45,75,57,0.05)' }}>
                        ...
                        <input
                          ...
                          onChange={e => {
                            ...
                            if (!isNaN(val) && val >= 1) {
                              setCartItems(items => items.map(i => i.id === item.id ? { ...i, quantity: val } : i));
                            }
                          }}
                          onBlur={e => {
                            const val = parseInt(e.target.value, 10);
                            if (isNaN(val) || val < 1) {
                              setCartItems(items => items.map(i => i.id === item.id ? { ...i, quantity: 1 } : i));
                            }
                          }}
                          className="text-center text-sm font-medium"
                          style={{
                            width: '40px', color: '#2D4B39', background: 'transparent',
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className="font-medium" style={{ color: '#2D4B39' }}>${(getPrecioEfectivo(item) * item.quantity).toLocaleString('es-CO')}</p>
                    {getDescuentoItem(item) && (
                      <p className="text-xs" style={{ color: '#10B981' }}>
                        Ahorro: ${((item.price - getPrecioEfectivo(item)) * item.quantity).toLocaleString('es-CO')}
                      </p>
                    )}
                  </div>
                </motion.div>
                  ...
                  {descuentoTotal > 0 && (
                    <div className="flex justify-between text-sm">
                      <span className="flex items-center gap-1" style={{ color: '#10B981' }}>
                        <Tag className="w-3 h-3" /> Descuentos aplicados
                      </span>
                      <span style={{ color: '#10B981', fontWeight: 600 }}>-${descuentoTotal.toLocaleString('es-CO')}</span>
                    </div>
                  )}
                  <div className="flex justify-between text-sm">
