...
import { Layers, Search, Plus, Info, Edit, Trash2, AlignLeft, Layout, Package } from 'lucide-react';
import { Modal } from './Modal';
import { DeleteConfirmModal } from './DeleteConfirmModal';
import { toast } from 'sonner';
import { categoriasInsumosAPI } from '../../lib/api';
      ...
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <div style={{ padding: '16px', background: 'rgba(184,134,11,0.06)', borderRadius: '12px', textAlign: 'center' }}>
            <div style={{ fontSize: '11px', color: '#9CA3AF', fontWeight: 600, marginBottom: '6px' }}>INSUMOS</div>
            <div style={{ fontSize: '28px', fontWeight: 800, color: '#B8860B' }}>{categoria.total_insumos || 0}</div>
          </div>
      ...
      <div style={{ marginBottom: '18px' }}>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input type="text" value={form.nombre} onChange={e => onChange('nombre', e.target.value)}
            placeholder="Ej: Hilos y Cuerdas" style={{ ...iStyle, paddingLeft: '38px' }} />
        </div>
      </div>
...
export function CategoriaInsumos() {
  ...
  const cargar = async () => {
    try {
      const data = await categoriasInsumosAPI.getAll();
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (modalType === 'add') {
        await categoriasInsumosAPI.create(form);
        toast.success('Categoría creada correctamente');
      } else if (modalType === 'edit' && selected) {
        await categoriasInsumosAPI.update(selected.id_categoria_insu, form);
  ...
  const toggleEstado = async (c: any) => {
    try {
      await categoriasInsumosAPI.update(c.id_categoria_insu, { estado: !c.estado });
      cargar();
  ...
  const confirmDelete = async () => {
    if (!toDelete) return;
    try {
      await categoriasInsumosAPI.delete(toDelete.id_categoria_insu);
  ...
  return (
    <motion.div initial={{ opacity: 0, translateY: 20 }} animate={{ opacity: 1, translateY: 0 }} style={{ marginBottom: '32px' }}>
      ...
      <motion.div initial={{ opacity: 0, translateY: -20 }} animate={{ opacity: 1, translateY: 0 }} transition={{ duration: 0.6 }}
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
            ...
            <div>
              <h1 style={{ fontSize: '30px', fontWeight: 700, color: '#2D4B39', margin: 0, marginBottom: '8px' }}>Categorías de Insumos</h1>
        ...
        {loading ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead style={{ background: '#2D4B39', color: '#fff' }}>
              <tr>
                {['CATEGORÍA', 'DESCRIPCIÓN', 'INSUMOS', 'ESTADO', 'ACCIONES'].map(h => (
                  <th key={h} style={{ padding: '16px 24px', textAlign: h === 'INSUMOS' || h === 'ESTADO' || h === 'ACCIONES' ? 'center' : 'left', fontSize: '12px', fontWeight: 600, letterSpacing: '0.05em' }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              <AnimatePresence>
                {filtered.map((cat, i) => (
                  <motion.tr key={cat.id_categoria_insu}
                    ...
                    <td style={{ padding: '18px 24px' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        <div style={{ width: '34px', height: '34px', borderRadius: '8px', background: 'linear-gradient(135deg,#2D4B39,#1a2f23)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <Layers style={{ width: '14px', height: '14px', color: '#fff' }} />
                        </div>
      ...
      {modalType && (
        <Modal isOpen={true} onClose={closeModal} title={modalType === 'add' ? 'Nueva Categoría de Insumo' : modalType === 'edit' ? 'Editar Categoría' : 'Información de Categoría'}>
